...
public class ColladaLoader {
	...
	//ini digunakan untuk mendapatkan sebagian animasi
	public static Animation loadAnimation(InputStream colladaFile) {

		//membaca animasi dalam
		AnimationData animationData = loadColladaAnimation(colladaFile);
		//AnimationData animationDataBayangan = 0;

		//membuat keyframe container untuk menampung keyframe yang ada
		//KeyFrame[] frames = new KeyFrame[animationData.keyFrames.length];
		KeyFrame[] frames = new KeyFrame[animationData.keyFrames.length];

		//memasukkan data animasi kedalam array
		for (int i = 0; i < frames.length; i++) {
			frames[i] = createKeyFrame(animationData.keyFrames[i]);
		}
		//menyimpan animasi yang telah dimasukkan dalam array
		return new Animation(animationData.lengthSeconds, frames);  //tidak bisa digunakan jika ada keyframe yang kosong
		//return new Animation(3, frames); //menentukan panjang animasi //harus sinkron dengan jumlah keyframe yang diambil
	}
