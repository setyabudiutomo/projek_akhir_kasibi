...
public class Animator {
    ...
    // TODO: implement slower/faster speed //berhasil di manipulas
	public float speed;
	...
	private void initAnimation(AnimatedModel animatedModel) {
		...
		// TODO: untuk menghilangkan sisa keyframe yang tidak dirender ada dalam collada loader
	...
	private void increaseAnimationTime(AnimatedModel obj) {
		...
		this.animationTime %= obj.getAnimation().getLength(); //diedit biar tidak loop
	}
	...
	//berdasarkan jumlah keyframe
	private KeyFrame[] getPreviousAndNextFrames(AnimatedModel obj) {
		int frame = 0; //untuk set frame sesuai dengan masukan dari activity coba
		KeyFrame[] allFrames = obj.getAnimation().getKeyFrames();
		//KeyFrame[] animasipilihan = new KeyFrame[frame]; //digunakan nanti
		//KeyFrame[] animasipilihan = new KeyFrame[5]; //ukurannya harus sama dengan frame yang ingin diambil

		//tinggal tidak merender bagian terdapat animasi

		/*for(int i = 0; i < 5; i++){
			animasipilihan[i] = allFrames[i];
			//dapat berjalan full dari frame 0 hingga 10
			//dapat berjalan dari 0-5
			//frame tidak bisa dipindah karena akan menyebabkan tidak muncul
		}*/

		// TODO: percobaan demi percobaan
		//start keyframe harus diubah
		//jika start tidak diubah maka ia akan selalu sampai memplay sampai habis
		KeyFrame previousFrame = allFrames[0];
		KeyFrame nextFrame = allFrames[0];

		/*KeyFrame previousFrame = animasipilihan[0];
		KeyFrame nextFrame = animasipilihan[0];*/
